...
## ⚠️ Always regenerate Prisma client before running tests

Any time you add or modify a migration (new model, new field, renamed field), run:

```bash
npx prisma generate
```

If you skip this, tests crash at runtime with `prisma.<model> is not a function` — the build passes but tests fail. **Run `npx prisma generate` before `npm test` whenever the schema changed.**

Also apply pending migrations to the test DB:
```bash
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/polla_test npx prisma migrate deploy
```
