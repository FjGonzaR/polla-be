import type { WorldCupMatch, WorldCupStanding, WorldCupTeam } from '../types/worldcup-api.types.js'
...
class WorldCupApiClient {
  private async fetch<T>(path: string): Promise<T> {
    ...
    try {
      ...
      throw new Error(`WorldCup API request failed for ${url}: ${(err as Error).message}`)
    } finally {
      clearTimeout(timer)
    }
  ...
  async getStandings(): Promise<WorldCupStanding[]> {
    const data = await this.fetch<{ groups: WorldCupStanding[] }>('/get/groups')
    return data.groups
  }
  ...
  async getMatch(externalMatchId: string): Promise<WorldCupMatch> {
    const data = await this.fetch<{ game: WorldCupMatch }>(`/get/game/${externalMatchId}`)
    return data.game
  }
