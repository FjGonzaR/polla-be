...
async function computeProvisionalKoPoints(
  ...
  const scaleFactors = new Map<string, number>(
    await Promise.all(
      uniqueSlugs.map(
        async (slug) => [slug, await getParam(slug)] as [string, number],
      ),
    ),
  );
...
async function computeProvisionalPowerupGroupPoints(
  participants: { id: string }[],
): Promise<Map<string, { darkHorse: number; disappointment: number }>> {
  const result = new Map<
    string,
    { darkHorse: number; disappointment: number }
  >();
  ...
  const [powerups, persisted, ptsDarkHorse, ptsDisappointment, scaleGroup] =
    await Promise.all([
      prisma.powerup.findMany(),
      prisma.scoreEvent.findMany({
        where: {
          paramKey: {
            in: ["pts_dark_horse_group", "pts_disappointment_group"],
          },
        },
  ...
  const persistedKeys = new Set(
    persisted.map((e) => `${e.participantId}:${e.paramKey}`),
  );
  ...
  for (const powerup of powerups) {
    ...
    if (
      ...
      darkHorse += Math.round(ptsDarkHorse * scaleGroup);
    }
    if (
      qualifiedTeamIds.has(powerup.disappointmentTeamId) &&
      !persistedKeys.has(`${powerup.participantId}:pts_disappointment_group`)
    ) {
      disappointment -= Math.round(ptsDisappointment * scaleGroup);
    }
    if (darkHorse !== 0 || disappointment !== 0) {
      result.set(powerup.participantId, { darkHorse, disappointment });
    }
  }
...
// Provisional group-position points (groups bucket) for groups whose standings are
// consolidated but not yet persisted into score_event. Shared by the scoreboard and the
// breakdown so both agree.
async function computeProvisionalGroupPoints(
  participants: { id: string }[],
): Promise<Map<string, number>> {
  const result = new Map<string, number>();
  ...
  if (provisionalGroupIds.length === 0) return result;
  ...
  const [ptsExact, bonusComplete, allGroupPredictions] = await Promise.all([
    ...
    prisma.groupPrediction.findMany({
      where: { groupId: { in: provisionalGroupIds } },
    }),
  ]);
  ...
  for (const { id: participantId } of participants) {
    ...
    for (const groupId of provisionalGroupIds) {
      ...
      const groupPreds = allGroupPredictions.filter(
        (p) => p.participantId === participantId && p.groupId === groupId,
      );
      if (groupPreds.length !== 4) continue;
      const exactCount = groupPreds.filter(
        (p) =>
          stds.find((s) => s.teamId === p.teamId)?.realPosition ===
          p.predictedPosition,
      ).length;
...
export async function getScoreboard(
  ...
  const [participants, scoreGroups, exactKoGroups] = await Promise.all([
    ...
    prisma.scoreEvent.groupBy({
      by: ["participantId"],
      where: { paramKey: "pts_ko_exact_score" },
      _count: { id: true },
    }),
  ]);

  const persistedPointsMap = new Map(
    scoreGroups.map((g) => [g.participantId, Number(g._sum.points ?? 0)]),
  );
  const exactKoMap = new Map(
    exactKoGroups.map((e) => [e.participantId, e._count.id]),
  );

  const [
    provisionalPointsMap,
    provisionalKoPointsMap,
    provisionalPowerupGroupMap,
  ] = await Promise.all([
    computeProvisionalGroupPoints(participants),
    computeProvisionalKoPoints(participants),
...
export async function getScoreboardBreakdown(
  ...
  const prize =
    scoreboard.data.find((e) => e.participant.id === participantId)?.prize ??
    null;

  // Add the same provisional contributions the scoreboard total uses, so breakdown matches.
  const onlyThisParticipant = [{ id: participantId }];
  const [provGroup, provKo, provPowerupGroup] = await Promise.all([
    computeProvisionalGroupPoints(onlyThisParticipant),
