  ...
  for (const { groupId, rankings } of predictions) {
    ...
    if (rankings.length !== 4) {
      throw new AppError(
        400,
        "INVALID_RANKINGS",
        "Debes enviar exactamente 4 posiciones",
      );
    }
    ...
    for (const { teamId } of rankings) {
      if (!groupTeamIds.has(teamId)) {
        throw new AppError(
          400,
          "INVALID_RANKINGS",
          `Equipo ${teamId} no pertenece al grupo`,
        );
      }
    }
    ...
    if (sorted.join() !== "1,2,3,4") {
      throw new AppError(
        400,
        "INVALID_RANKINGS",
        "Posiciones inválidas o duplicadas",
      );
    }
...
function computeGroupStatus(
  ...
  const data: GroupPredictionStatus[] = groups.map((group) => {
    ...
    const groupPredictionCompleted = groupPreds.length === 4;
    ...
    if (groupPredictionCompleted) {
      ...
      if (ledger) {
        ...
        if (
          standings.length === 4 &&
          standings.every((s) => s.realPosition !== null) &&
          standings.some((s) => s.matchesPlayed > 0)
        ) {
          const exactCount = rankings.filter(
            (r) =>
              standings.find((s) => s.teamId === r.teamId)?.realPosition ===
              r.predictedPosition,
          ).length;
          const pts = exactCount * ptsExact;
          const bonus = exactCount === 4 ? bonusComplete : 0;
          pointsEarned = {
            pts_group_position_exact: pts,
            bonus_group_complete: bonus,
            total: pts + bonus,
          };
        }
      }
    ...
    return {
      ...
      label: group.label,
      name: group.name,
      groupComplete: groupPredictionCompleted,
      rankings,
      pointsEarned,
    };
  });
...
function buildLedgerGroupMap(
  ...
  for (const e of events) {
    ...
    const curr = map.get(e.groupId) ?? {
      pts_group_position_exact: 0,
      bonus_group_complete: 0,
    };
    if (e.paramKey === "pts_group_position_exact")
      curr.pts_group_position_exact += e.points;
    if (e.paramKey === "bonus_group_complete")
      curr.bonus_group_complete += e.points;
...
export async function findFriendsGroupPredictions(
  ...
  if (friendId) {
    if (friendId === participantId) {
      throw new AppError(
        400,
        "INVALID_FRIEND",
        "Cannot request predictions for yourself",
      );
    }
    const friend = await prisma.participant.findUnique({
      where: { id: friendId },
      select: { id: true },
    });
    if (!friend)
  ...
  for (const e of allLedgerEvents) {
    if (!participantLedger.has(e.participantId))
      participantLedger.set(e.participantId, new Map());
    const gMap = participantLedger.get(e.participantId)!;
    const curr = gMap.get(e.groupId ?? "") ?? {
      pts_group_position_exact: 0,
      bonus_group_complete: 0,
    };
    if (e.paramKey === "pts_group_position_exact")
